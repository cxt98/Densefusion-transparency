...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num_pt, add_noise, root, noise_trans, refine):
        if mode == 'train':
            self.path = 'datasets/unreal/dataset_config/train_data_list.txt' 
        elif mode == 'test':
            self.path = 'datasets/unreal/dataset_config/test_data_list.txt'  
        self.num_pt = num_pt
        self.root = root
        ...
        while 1:
            ...
            if mode == 'train':
                self.list.append('unreal_data_example/Room_Capturer_2020.04.11-15.36.39/' + input_line)
            else:
                self.list.append('unreal_data_example/Room_Capturer_2020.04.11-15.36.39/' + input_line)
        ...
        class_file = open('datasets/unreal/dataset_config/classes.txt')
        class_id = 1
        self.cld = {}
        while 1:
    ...
    def __getitem__(self, index):
        img = Image.open(self.root+self.list[index]+'.png')
        ...
        if img.width == 1024:
            ...
            self.xmap = self.xmap2
            self.ymap = self.ymap2
        else:
            cam_cx = self.cam_cx_1
            cam_cy = self.cam_cy_1
            cam_fx = self.cam_fx_1
            cam_fy = self.cam_fy_1
            self.xmap = self.xmap1
            self.ymap = self.ymap1
        ...
        if self.add_noise:
            for k in range(5):
                seed = random.choice(self.list)
                front = np.array(self.trancolor(Image.open(self.root+seed+'.png').convert("RGB")))
                front = np.transpose(front, (2, 0, 1))
                f_label = np.array(Image.open(seed+'.is.png'))
                front_label = np.unique(f_label).tolist()[1:]
        ...
        seed = random.choice(self.list)
        back = np.array(self.trancolor(Image.open(self.root+seed+'.png').convert("RGB")))
        ...
        cam_scale = 1.0  # Depth factor (not 1.0!!!)
