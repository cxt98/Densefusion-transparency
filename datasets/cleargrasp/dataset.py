...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num_pt, add_noise, root, noise_trans, refine):
        ...
        class_file = open('datasets/cleargrasp/dataset_config/classes.txt')
        class_id = 1
        self.cld = {}
        while 1:
        ...
        self.cam_cx_1 = 960
        self.cam_cy_1 = 540
        self.cam_fx_1 = 1386.4
        self.cam_fy_1 = 1386.4
    ...
    def list2realpath(self, list, param, suffix):
        if self.mode == 'train':
            object_path = list.split('/')[0]
            modality_path = list.split('/')[-2]
            file_id = list.split('/')[-1]
            return '{0}/{1}/{2}/{3}{4}'.format(self.root, object_path + '/' + modality_path, param, file_id, suffix)
        elif self.mode == 'test':
            object_path = list.split('/')[0] + '/' + list.split('/')[1]
            modality_path = list.split('/')[-2]
            file_id = list.split('/')[-1]
            return '{0}/{1}/{2}/{3}{4}'.format(self.root, object_path + '/' + modality_path, param, file_id, suffix)
